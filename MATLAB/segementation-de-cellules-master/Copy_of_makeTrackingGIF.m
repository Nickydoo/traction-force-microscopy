...
function tracks = Copy_of_makeTrackingGIF(rawDataDir,tracks,resDir,frames,msk, trkFilter,positionID)
...
%TEST JOANNIE - Draw tracks on second last frame 
clf(fh,'reset');

im = imread(fullfile(rawDataDir, fnIms(frames(end-1)).name));
...
for t = 1:numel(idsInTimeRange)
    iTrack = tracksGood(ismember(tracksGood(1:(end/2),4),idsInTimeRange(t)),:);
    
    msk = ismember(iTrack(:,3),frames(end-1));
...
for t = 1:numel(idsInTimeRange)
    iTrack = tracksGood(ismember(tracksGood((end/2):end,4),idsInTimeRange(t)),:);
