function [costMat,propagationScheme,kalmanFilterInfoFrame2,nonlinkMarker,...
    errFlag] = costMatRandomDirectedSwitchingMotionLinkSpatialAverage(movieInfo,kalmanFilterInfoFrame1,...
...
% [JM] Averaged in space nearest neighbour  
    ...
    if isfield(costMatParam,'avgSz')
        avgSz  = costMatParam.avgSz;
    else
        warning('avgSz not Defined !')
    end

    if isfield(costMatParam,'imsz')
        imsz   = costMatParam.imsz;
    else
        warning('imsz not Defined !')
    end
    
    persistent oPos nnMap;

    %The map is computed only once
    if (currentFrame == 1 || isempty(oPos) || isempty(nnMap))
...
% [JM] Averaged in space nearest neighbour
    ...
    thisPos = oPos(oPos(:,3) == currentFrame,:);
    ix      = sub2ind(imsz, round(thisPos(:,2)), round(thisPos(:,1))); 
...
if useLocalDensity
    ...
    % [JM]  Use averaged-in-space nearest-neighbour
    ratioDist2Std = nnDistAverage./kalmanStd/closestDistScale;
    % [\JM] Use averaged-in-space nearest-neighbour
