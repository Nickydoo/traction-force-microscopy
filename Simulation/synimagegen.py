        ...
        for c in range(bl, br):
            for r in range(bu, bd):
                IMAGE_OUT[r, c] = IMAGE_OUT[r, c] + max_int * par_diam ** 2 * np.pi / 32 * \
                                  (scipy.special.erf(sqrt8 * (c - x - 0.5) / par_diam) - scipy.special.erf(
                                      sqrt8 * (c - x + 0.5) / par_diam)) * \
                                  (scipy.special.erf(sqrt8 * (r - y - 0.5) / par_diam) - scipy.special.erf(
                                      sqrt8 * (r - y + 0.5) / par_diam))
