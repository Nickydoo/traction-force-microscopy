...
"""
OpenPIV algorithm adapted to use individual bead tracking algorithm, very slow and not recommended
"""
...
def get_coordinates_maxima(img):
    coords = peak_local_max(img, threshold_abs=2*np.average(img), min_distance=10)
    x = coords[:, 1]
    y = coords[:, 0]
    return np.meshgrid(x, y)
...
def extended_search_area_piv_sb(
        frame_a, frame_b,
        window_size,
        overlap=0,
        dt=1.0,
    ...
    coords = peak_local_max(frame_a, threshold_abs=2*np.average(frame_a), min_distance=10)
    row = coords[:, 0]
    col = coords[:, 1]


    u, v = np.zeros((len(row), len(col))), np.zeros((len(row), len(col)))

    # if we want sig2noise information, allocate memory
    if sig2noise_method is not None:
        sig2noise = np.zeros((len(row), len(col)))
    ...
    for k in row:
        ...
        for m in col:
            ...
            if np.any(window_a):
                ...
                rowa, cola = find_subpixel_peak_position(corr,
                                                       subpixel_method=subpixel_method)

                rowa -= (search_area_size + window_size - 1) // 2
                cola -= (search_area_size + window_size - 1) // 2

                # get displacements, apply coordinate system definition
                u[k, m], v[k, m] = -cola, rowa
